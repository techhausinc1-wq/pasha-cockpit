...
export interface InboundContext {
  ...
  thread_history: Array<{ from: "customer" | "worker"; text: string; at: string }>;
...
- Open Mon-Sat 9am-7pm, Sun 10am-6pm (today closes at ${ctx.store_open_until || "7pm"})
- Phone (210) 712-4700
...
function buildUserPrompt(ctx: InboundContext): string {
  ...
  lines.push(`Customer: ${ctx.customer_name}${ctx.customer_handle ? " (" + ctx.customer_handle + ")" : ""}`);
  ...
  if (ctx.product_match) {
    ...
    if (ctx.in_stock !== undefined) {
      lines.push(`- Stock: ${ctx.in_stock === true ? "in stock" : ctx.in_stock === false ? "not in stock" : "unknown — confirm with worker"}`);
    }
    lines.push("");
  }
  ...
  lines.push("");
  lines.push(`Compose the draft reply. Return STRICT JSON per the schema in the system prompt.`);
...
export async function draftReply(ctx: InboundContext): Promise<DraftResult> {
  ...
  try {
    ...
    if (!match) throw new Error("Drafter LLM did not return parseable JSON: " + raw.slice(0, 200));
    parsed = JSON.parse(match[0]);
  }

  // Defensive defaults
  if (!parsed.intent) parsed.intent = classifyByRules(ctx.latest_message).intent;
