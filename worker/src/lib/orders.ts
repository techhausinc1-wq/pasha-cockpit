// Deposit / balance-due ledger for special orders — the piece every
// established furniture CRM (STORIS, MicroBiz, Deelo) treats as core and
// this cockpit didn't have yet. Financing applications (SAMPLE_APPLICATIONS
// in sample-data.ts) track lender status; this tracks the cash side: what
// the customer put down, what's still owed, and whether the piece has
// actually arrived from the supplier.
//
// Loosely linked to TRUCKS_INBOUND by supplier name — no hard foreign key
// since trucks carry many customers' orders and this is still shell-phase,
// in-memory data.

import { recordPurchase } from "./customers.ts";

export type OrderStatus =
  | "deposit-paid"
  | "awaiting-truck"
  | "ready-for-delivery"
  | "delivered"
  | "cancelled";

export interface Order {
  id: string;
  customer_id: string;
  customer_name: string; // denormalized for display without a join
  product_slugs: string[];
  total_amount: number;
  deposit_amount: number;
  balance_due: number;
  status: OrderStatus;
  supplier: "Crown Mark" | "Happy Homes" | "In stock" | "Other";
  worker_id: string;
  created_at: string;
  updated_at: string;
}

export const SAMPLE_ORDERS: Order[] = [
  {
    id: "o_1",
    customer_id: "c_joel_bryant",
    customer_name: "Joel Bryant",
    product_slugs: [
      "amelia-charcoal-power-reclining-sectional-w-bluetooth-speakers",
    ],
    total_amount: 2199,
    deposit_amount: 2199,
    balance_due: 0,
    status: "awaiting-truck",
    supplier: "Crown Mark",
    worker_id: "u_carlos",
    created_at: new Date(Date.now() - 22 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 22 * 60 * 1000).toISOString(),
  },
  {
    id: "o_2",
    customer_id: "c_maria_lopez",
    customer_name: "Maria Lopez",
    product_slugs: ["logan2-charcoal-reversible-sectional-charcoal"],
    total_amount: 1899,
    deposit_amount: 300,
    balance_due: 1599,
    status: "deposit-paid",
    supplier: "Happy Homes",
    worker_id: "u_rick",
    created_at: new Date(Date.now() - 8 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 8 * 60 * 1000).toISOString(),
  },
];

export function findOrderById(id: string): Order | undefined {
  return SAMPLE_ORDERS.find((o) => o.id === id);
}

export function ordersForCustomer(customerId: string): Order[] {
  return SAMPLE_ORDERS.filter((o) => o.customer_id === customerId);
}

export function createOrder(input: {
  customer_id: string;
  customer_name: string;
  product_slugs: string[];
  total_amount: number;
  deposit_amount: number;
  supplier: Order["supplier"];
  worker_id: string;
}): Order {
  const now = new Date().toISOString();
  const order: Order = {
    id: `o_${Date.now()}_${Math.floor(Math.random() * 1000)}`,
    customer_id: input.customer_id,
    customer_name: input.customer_name,
    product_slugs: input.product_slugs,
    total_amount: input.total_amount,
    deposit_amount: input.deposit_amount,
    balance_due: Math.max(0, input.total_amount - input.deposit_amount),
    status: input.deposit_amount >= input.total_amount
      ? "awaiting-truck"
      : "deposit-paid",
    supplier: input.supplier,
    worker_id: input.worker_id,
    created_at: now,
    updated_at: now,
  };
  SAMPLE_ORDERS.push(order);
  // The deposit itself counts toward lifetime value immediately — full
  // balance gets added when recordBalancePayment closes it out.
  recordPurchase(input.customer_id, input.deposit_amount, now);
  return order;
}

export function recordBalancePayment(
  orderId: string,
  amount: number,
): Order | null {
  const order = findOrderById(orderId);
  if (!order) return null;
  order.balance_due = Math.max(0, order.balance_due - amount);
  order.updated_at = new Date().toISOString();
  if (order.balance_due === 0 && order.status === "deposit-paid") {
    order.status = "awaiting-truck";
  }
  recordPurchase(order.customer_id, amount, order.updated_at);
  return order;
}

export function updateOrderStatus(
  orderId: string,
  status: OrderStatus,
): Order | null {
  const order = findOrderById(orderId);
  if (!order) return null;
  order.status = status;
  order.updated_at = new Date().toISOString();
  return order;
}
